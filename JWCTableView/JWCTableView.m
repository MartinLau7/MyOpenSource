...
#import "TTMainViewTableRowView.h"

static NSInteger globalNumberOfRows = 0;

@implementation JWCTableView

#pragma mark - key down
- (void)keyDown:(NSEvent *)theEvent{
    if (_jwcTableViewKeyDownDelegate != nil && [_jwcTableViewKeyDownDelegate conformsToProtocol:@protocol(JWCTableViewKeyDownDelegate)] && [_jwcTableViewKeyDownDelegate respondsToSelector:@selector(didKeyDown:event:)]) {
        [_jwcTableViewKeyDownDelegate didKeyDown:self event:theEvent];
    }
...
- (void)mouseDown:(NSEvent *)theEvent{
    if (_jwcTableViewKeyDownDelegate != nil && [_jwcTableViewKeyDownDelegate conformsToProtocol:@protocol(JWCTableViewKeyDownDelegate)] && [_jwcTableViewKeyDownDelegate respondsToSelector:@selector(didMouseDown:event:)]) {
        [_jwcTableViewKeyDownDelegate didMouseDown:self event:theEvent];
    }
    [super mouseDown:theEvent];
}


#pragma mark - right click at row
- (NSMenu *)menuForEvent:(NSEvent *)event{
    NSInteger row = [self rowAtPoint:[self convertPoint:event.locationInWindow fromView:nil]];
    NSIndexPath *indexPath = [self tableView:self indexPathForRow:row];
    //如果点击无效,或者点击到section
    if (row == -1 || indexPath.row == -1){
        return nil;
    }
    if (_jwcTableViewRightClickDelegate != nil && [_jwcTableViewRightClickDelegate respondsToSelector:@selector(rightClickMenuAtTableView:andIndexPath:)]) {
        return [_jwcTableViewRightClickDelegate rightClickMenuAtTableView:self andIndexPath:indexPath];
    }
    return nil;
}
...
- (NSTableRowView *)tableView:(NSTableView *)tableView rowViewForRow:(NSInteger)row{
    if (_jwcTableViewDelegate != nil && [_jwcTableViewDelegate respondsToSelector:@selector(tableView:rowViewForRow:)]) {
        return [_jwcTableViewDelegate tableView:tableView rowViewForRow:row];
    }
    return nil;
}
...
- (NSView *)cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSInteger numberOfRows =  [self getNumberOfRowsAtIndexPath:indexPath];
    
    if (numberOfRows > [self numberOfRows] - 1) {
        return nil;
    }
    
    return [self viewAtColumn:0 row:numberOfRows makeIfNecessary:YES];
}
...
- (CGRect)rectForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    return [self frameOfCellAtColumn:0 row:numberOfRows];
}
...
- (void)noteHeightOfRowChangedWithIndexPath:(NSIndexPath *)indexPath{
    ...
    [self noteHeightOfRowsWithIndexesChanged:[NSIndexSet indexSetWithIndex:numberOfRows]];
}

//numbers
- (NSInteger)numberOfRowsInSection:(NSInteger)section{
    return [self.jwcTableViewDataSource tableView:self numberOfRowsInSection:section];
}

#pragma mark - private get number of rows
- (NSInteger) getNumberOfRowsAtIndexPath:(NSIndexPath *)indexPath{
    globalNumberOfRows = 0;
    NSInteger numberOfPreSectionRows = 0;
    NSInteger numberOfCurSectionRows = 0;
    
    for (NSInteger i = 0; i < indexPath.section; i ++) {
        
        NSInteger num = [self.jwcTableViewDataSource tableView:self numberOfRowsInSection:i];
        
        if ([self.jwcTableViewDataSource tableView:self hasHeaderViewForSection:i]) {
            numberOfPreSectionRows = 1 + num;
        } else {
            numberOfPreSectionRows += num;
        }
        
        globalNumberOfRows += numberOfPreSectionRows;
        numberOfPreSectionRows = 0;
    }
    
    //add current section
    if ([self.jwcTableViewDataSource tableView:self hasHeaderViewForSection:indexPath.section]) {
        numberOfCurSectionRows = 1 + indexPath.row;
    } else {
        numberOfCurSectionRows += indexPath.row;
    }
    
    globalNumberOfRows += numberOfCurSectionRows;
    
    return MAX(globalNumberOfRows, 0);
}
